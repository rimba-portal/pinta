    ...
    public static function make(WorkflowAction|string $workflowAction): Action
    {
        ...
        return Action::make($workflowAction->value)
            ...
            ->schema($workflowAction === WorkflowAction::Assign ? [] : [Textarea::make('comment')
                ->maxLength(2000)])
            ->visible(fn (WorkflowInstance $record): bool => in_array($workflowAction, app(WorkflowEngine::class)
                ->availableActions($record)))
            ->action(fn (WorkflowInstance $record, array $data) => app(WorkflowEngine::class)
                ->perform($record, $workflowAction, auth()->user(), $data));
    }

    public static function all(): array
    {
        return array_map(fn (WorkflowAction $a): Action => self::make($a), WorkflowAction::cases());
    }
}
